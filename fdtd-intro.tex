...
O método FDTD utiliza diferenças finitas como uma aproximação tanto 
...
\end{equation}
Assim, o termo à esquerda é igual à derivada da função
no ponto $ x_0 $ mais um termo que depende de $ \ delta ^ 2 $ mais um
número infinito de outros termos que não são mostrados. Para os termos
que não são mostrados, o próximo dependeria de $ \ delta ^ 4 $ e todos
os termos subsequentes dependeriam de potências ainda maiores de $ \ delta $.
Com um pouco de arrumação, essa relação é pode ser escrita como
\begin{equation}
  \left.\frac{df(x)}{dx}\right|_{x=x_0} = 
  \frac{f\!\left(x_0+\frac{\delta}{2}\right) -
        f\!\left(x_0-\frac{\delta}{2}\right)}{\delta} + O(\delta^2).
\end{equation}
O termo "$O$" representa todos os termos que não são explicitamente
mostrados e o valor entre parênteses, ou seja, $ \ delta ^ 2 $, indica a
ordem mais baixa de $ \ delta $ nesses termos ocultos. Se $ \ delta $ é
suficientemente pequeno, uma aproximação razoável da derivada pode
ser obtida simplesmente ignorando todos os termos representados pelo
termo "$O$". Assim, a aproximação da diferença central é dada
por
\begin{equation}
  ...
  \frac{f\!\left(x_0+\frac{\delta}{2}\right) -
        f\!\left(x_0-\frac{\delta}{2}\right)}{\delta}.
\end{equation}
Perceba que a diferença central nos dá uma aproximação da
derivada da função em $x_0$, mas a função não é realmente 
tomada neste ponto. Ao invés disso, a função é tomada nos pontos 
vizinhos $x_0+\delta/2$ e $x_0-\delta/2$. Como a menor potência de 
$\delta$ sendo ignorada é de segunda ordem, a diferença central é indicada 
como tendo uma precisão de segunda ordem (ou um comportamento de segunda ordem). 
Isto implica que se $\delta$ é reduzida por um fator de $10$, o erro na 
aproximação deveria ser reduzido por um fator de $100$ (no mínimo).
 No limite onde $\delta$ vai para zero, a 
 aproximação se torna exata.

É possível escrever diferenças centrais de ordens mais altas. Para obter
comportamento de tais ordens, i.e., mais pontos termos, precisamos usar
mais pontos de amostragem. O apêndice \ref{ap:diff} mostra a construção de 
uma diferença central de quarta ordem. A utilização de diferenças centrais de 
ordem mais alta em FDTD é possível, certamente. Mas existem algumas 
complicações que surgem por causa do crescente ``stencil'' do 
operador diferença. Por exemplo, quando um PEC está presente, é 
possível que o operador diferença se extenda para dentro do PEC 
prematuramente ou se extenda para depois da camada do PEC. 
Por esses tipos de problemas, consideraremos apenas o uso das 
